...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateActiveSection() {
        ...
        // Check if we're at the hero/home section (special case for top of page)
        if (scrollPosition < viewportHeight * 0.3) {
            currentSection = 'home';
        } else {
            // Check other sections - find which section takes up > 70% of viewport
            ...
            sections.forEach(section => {
                ...
                // Only consider this section if it occupies more than 70% of viewport
                if (viewportPercentage > 70 && viewportPercentage > maxVisiblePercentage) {
                    maxVisiblePercentage = viewportPercentage;
                    maxVisibleSection = section.getAttribute('id');
                }
            });
            
            // If we found a section that takes up > 70% of viewport, use it
    ...
    // --- Healthcare is Broken Expand/Collapse Functionality ---
    const healthcareCards = document.querySelectorAll('.broken-healthcare-card');
    
    // Function to expand or collapse all cards
    function toggleAllCards(shouldExpand) {
        healthcareCards.forEach(card => {
    ...
    healthcareCards.forEach(card => {
        ...
        if (expandBtn) {
            expandBtn.addEventListener('click', (e) => {
                ...
                // Check current state of ANY card to determine action
                const anyExpanded = Array.from(healthcareCards).some(
                    c => c.getAttribute('data-expanded') === 'true'
                );
                
                // Toggle all cards (if any are expanded, collapse all; otherwise expand all)
                toggleAllCards(!anyExpanded);
            });
        ...
        // Optional: Allow clicking anywhere on the card to expand/collapse all
    ...
    if (learnMoreButton) {
        learnMoreButton.addEventListener('click', (e) => {
            e.preventDefault();
            smoothScrollTo('#healthcare-broken', 80);
        });
    }
